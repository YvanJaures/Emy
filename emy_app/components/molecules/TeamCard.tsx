...
export default function TeamCard({
  ...
  const handleCle = () => {
    setError("");
    if (!cle.trim()) {
      setError("veuillez remplir le champs!");
      return false;
    }
    if (cle !== team.key_team) {
      setError("Clé incorrecte! Réessayer");
      return false;
    }
    setOnJoin(false);
    return true;
      ...
      {onJoin && (
        <div className="mt-1 flex flex-col items-center justify-start gap-2">
          <input
            ...
            name="cle"
            id="cle"
            className="w-full h-5 rounded-sm p-2 text-[13px] outline-blue-500"
          ...
          <span className="flex justify-evenly items-center w-full">
            <button
              className="bg-blue-500 text-white text-sm p-0.5 rounded-sm hover:bg-blue-600 hover:cursor-pointer"
              onClick={handleJoin}
            >
              Valider
            </button>
            <button
              className="bg-red-500 text-white text-sm p-0.5 rounded-sm hover:bg-red-600 hover:cursor-pointer"
              onClick={() => {
                setOnJoin(false);
                setError("");
                setCle("");
