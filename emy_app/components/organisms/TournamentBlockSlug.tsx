...
export default function TournamentBlockSlug({tournament}:Props){
    ...
    const etat:number=useMemo(()=>{
        ...
        {
            console.log('debut')
            const start=new Date(tournament?.start_date? tournament?.start_date:'00/00/0000')
            const end=new Date(tournament?.end_date? tournament?.end_date:'00/00/0000')
            ...
            <div className="flex flex-col">
                ...
                <span className="flex-10">
                    <ImageDefault
                        ...
                        avatar={tournament ? tournament?.avatar:'/assets/arrieres_plan/AlpineLake.png'}
                ...
                <section className="flex-20 p-2 gap-2 flex border-b ml-2 mr-2">
                    <section className="flex-80 flex flex-col text-gray-400 gap-2">
                        <p>
                            {tournament?.Community.details}
                        </p>
                        <span className="flex">
                            <p className="">
                            groupes : {tournament?.members? tournament?.members:'0'} <br />
                            début: {new Date(tournament? tournament?.start_date:'0/00/0000').toLocaleDateString()} <br />
                            fin: {new Date(tournament? tournament?.end_date:'0/00/0000').toLocaleDateString()} <br />
                            status: 
                            {
                                etat===-2 &&
                    ...
                    <span className="flex-20 h-full text-lg">
                        <p className="flex justify-start items-center gap-2">
                            <LuUsers
                                className=""/> {tournament?.Player?.length ?? 0}
                        </p>
                        <a href={`https://www.google.com/maps/place/${tournament?.location ?? '/'}`} target="_blank" rel="noopener noreferrer"
                            ...
                            <GrMapLocation 
                                className="hover:cursor-pointer hover:text-[#0F70AC]"/>
